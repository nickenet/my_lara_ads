	...
	<aside>
		<div class="inner-box">
			...
			@if(isset($cat))
				...
				<div id="sub_cat_list" class="categories-list list-filter">
					...
					<ul class="list-unstyled">
						<li>
							<a href="{{ lurl(trans('routes.t-search-cat').'/'.$cats->get($parent_id)->slug) }}"
							...
							<ul class="list-unstyled long-list">
								@foreach ($cats->groupBy('parent_id')->get($parent_id) as $sub_cat)
									<li>
										@if($uriPathSubCatSlug == $sub_cat->slug)
											<strong>
												<a href="{{ lurl(trans('routes.t-search-cat').'/'.$cats->get($sub_cat->parent_id)->slug.'/'.$sub_cat->slug) }}"
												   title="{{ $sub_cat->name }}">
			...
			<div id="cat_list" class="categories-list list-filter" <?php echo (isset($style)) ? $style : ''; ?>>
				...
				<ul class=" list-unstyled">
					@foreach ($cats->groupBy('parent_id')->get(0) as $cat)
						<li>
							@if ($uriPathCatSlug == $cat->slug)
								<strong>
									<a href="{{ lurl(trans('routes.t-search-cat').'/'.$cat->slug) }}"
									   title="{{ $cat->name }}">
			...
			<div class="locations-list list-filter">
				...
				<ul class="browse-list list-unstyled long-list">
					@foreach ($cities as $city)
						@if ($uriPathCityName == rawurlencode($city->name))
							<li>
								<strong>
									<a href="{{ lurl(trans('routes.t-search-location').'/'.rawurlencode($city->name)) }}"
									   title="{{ $city->name }}">
										{{ $city->name }}
									</a>
								</strong>
			...
			<div class="locations-list list-filter">
				...
				<ul class="browse-list list-unstyled long-list">
					<li><a href="{!! qsurl($fullUrl, array_merge(Request::except('page'), ['new'=>'1'])) !!}">{{ t('New') }} <span
									class="count"></span></a></li>
					<li><a href="{!! qsurl($fullUrl, array_merge(Request::except('page'), ['new'=>'0'])) !!}">{{ t('Used') }} <span
									class="count"></span></a></li>
					<li><a href="{!! qsurl($fullUrl, array_merge(Request::except('page'), ['new'=>''])) !!}">{{ t('None') }} </a></li>
				</ul>
			</div>
