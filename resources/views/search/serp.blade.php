	...
	<div class="main-container">
		<div class="container">
			<div class="row">
				...
				@include('search.inc.sidebar-left')
				...
				<div class="{{ $contentColSm }} page-content col-thin-left">
					<div class="category-list">
						<div class="tab-box ">
							...
							<ul id="ad_type" class="nav nav-tabs add-tabs" role="tablist">
								<li {!! $classAllActive !!}>
									<a href="{!! qsurl($fullUrl, Request::except(['page', 'type'])) !!}" role="tab" data-toggle="tab">
										{{ t('All Ads') }} <span class="badge {!! $classAllBadge !!}">{{ $count->get('all') }}</span>
									</a>
								</li>
								<li {!! $classProActive !!}>
									<a href="{!! qsurl($fullUrl, array_merge(Request::except('page'), ['type'=>2])) !!}" role="tab"
									   data-toggle="tab">
										{{ t('Professionals') }} <span class="badge {!! $classProBadge !!}">{{ $count->get('b') }}</span>
									</a>
								</li>
								<li {!! $classPersoActive !!}>
									<a href="{!! qsurl($fullUrl, array_merge(Request::except('page'), ['type'=>1])) !!}" role="tab"
									   data-toggle="tab">
							...
							<div class="tab-filter" style="padding-top: 6px; padding-right: 6px;">
								<select id="order_by" class="selecter" data-style="btn-select" data-width="auto">
									<option value="{!! qsurl($fullUrl, Request::except(['orderBy', 'distance'])) !!}">{{ t('Sort by') }}</option>
									<option{{ (Request::get('orderBy')=='priceAsc') ? ' selected="selected"' : '' }} value="{!! qsurl($fullUrl, array_merge(Request::except('orderBy'), ['orderBy'=>'priceAsc'])) !!}">{{ t('Price : Low to High') }}</option>
									<option{{ (Request::get('orderBy')=='priceDesc') ? ' selected="selected"' : '' }} value="{!! qsurl($fullUrl, array_merge(Request::except('orderBy'), ['orderBy'=>'priceDesc'])) !!}">{{ t('Price : High to Low') }}</option>
									<option{{ (Request::get('orderBy')=='relevance') ? ' selected="selected"' : '' }} value="{!! qsurl($fullUrl, array_merge(Request::except('orderBy'), ['orderBy'=>'relevance'])) !!}">{{ t('Relevance') }}</option>
									<option{{ (Request::get('orderBy')=='date') ? ' selected="selected"' : '' }} value="{!! qsurl($fullUrl, array_merge(Request::except('orderBy'), ['orderBy'=>'date'])) !!}">{{ t('Date') }}</option>
									@if ($isLocSearch or Request::has('l') or Request::has('r'))
										<option{{ (Request::get('distance')==100) ? ' selected="selected"' : '' }} value="{!! qsurl($fullUrl, array_merge(Request::except('distance'), ['distance'=>100])) !!}">{{ t('Around') . ' 100 km' }}</option>
										<option{{ (Request::get('distance')==300) ? ' selected="selected"' : '' }} value="{!! qsurl($fullUrl, array_merge(Request::except('distance'), ['distance'=>300])) !!}">{{ t('Around') . ' 300 km' }}</option>
										<option{{ (Request::get('distance')==500) ? ' selected="selected"' : '' }} value="{!! qsurl($fullUrl, array_merge(Request::except('distance'), ['distance'=>500])) !!}">{{ t('Around') . ' 500 km' }}</option>
									@endif
								</select>
							</div>
						...
						<div class="listing-filter">
							<div class="pull-left col-xs-6">
								<div class="breadcrumb-list">
									<a href="{{ lurl(trans('routes.v-search', ['countryCode' => $country->get('icode')])) }}" class="current">
										<span>{{ t('All ads') }}</span>
									</a>
									@if ($isLocSearch or Request::has('l') or Request::has('r'))
						...
						<div class="tab-box save-search-bar text-center">
							@if(Request::has('q') and Request::get('q') != '' and $count->get('all') > 0)
								<a name="{!! qsurl($fullUrl, Request::except(['_token', 'location'])) !!}" id="save_search"
								   count="{{ $count->get('all') }}">
					...
					<div class="post-promo text-center" style="margin-bottom: 30px;">
						...
						<a href="{{ lurl(trans('routes.create')) }}" class="btn btn-lg btn-border btn-post btn-danger">{{ t('Start Now!') }}</a>
					</div>
