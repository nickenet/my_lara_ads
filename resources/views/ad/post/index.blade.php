					...
					<div class="inner-box category-content">
						...
						<div class="row">
							<div class="col-sm-12">
								...
								<form class="form-horizontal" id="createAd-1" method="POST" action="{{ lurl('create/submit') }}"
									...
									<fieldset>
										...
										<div id="conditionBloc" class="form-group required <?php echo ($errors->has('new')) ? 'has-error' : ''; ?>">
											<label class="col-md-3 control-label">{{ t('Condition') }} <sup>*</sup></label>
											<div class="col-md-8">
												<select name="new" id="new" class="form-control selecter">
													@foreach ([0 => t('Used'), 1 => t('New')] as $key => $value)
														<option value="{{ $key }}"
																@if(old('new')==$key)selected="selected"@endif> {{ $value }} </option>
													@endforeach
												</select>
											</div>
										</div>
...
@section('javascript')
	...
	<script src="{{ url('/assets/js/app/d.select.category.js?time=' . time()) }}"></script>
