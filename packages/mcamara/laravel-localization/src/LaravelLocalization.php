...
class LaravelLocalization
{
    ...
    public function __construct()
    {
        ...
        // For LaraClassified - Default language (from Admin panel)
        try {
        ...
        if (!empty($defaultLang)) {
            ...
            $this->defaultLocale = $defaultLang->abbr;
        } else {
	 ...
	 * @param  array $params For LaraClassified.
     *
     *
     * @return string|false                URL translated, False if url does not exist
     */
    public function getLocalizedURL($locale = null, $url = null, $attributes = array(), $params = array())
    {
        if ($locale === null) {
        ...
        if (empty($url)) {
			...
			$preUri = '';
			if (!empty($defaultLang)) {
				if (!(config('laravellocalization.hideDefaultLocaleInURL')==true && $locale==$defaultLang->abbr)){
					$preUri = $locale . '/';
				}
			}
    ...
    protected function unparseUrl( $parsed_url )
    {
        ...
        // For LaraClassified : Sub-folder support. Added at version 1.3
